...
typedef struct tag%PFX%_t { int mode; } %PFX%_t;

%PFX%_t *%PFX%_init(const char *fncfg, unsigned fromcpt,
    gmx_mtop_t *mtop, t_inputrec *ir, t_commrec *cr, int mode)
{
  %PFX%_t *%OBJ%;
  if (!(cr->duty & DUTY_PP)) return NULL; /* skip a PME-only node */
  if ((%OBJ% = calloc(1, sizeof(*%OBJ%))) == NULL) exit(1);
  if (SIMMASTER(cr)) %OBJ%->mode = mode;
  if (PAR(cr)) gmx_bcast(sizeof(*%OBJ%), %OBJ%, cr);
  return %OBJ%;
}

void %PFX%_done(%PFX%_t *%OBJ%) {}

int %PFX%_move(%PFX%_t *%OBJ%, gmx_enerdata_t *enerd,
...
txtdecl = r'''/* generated by gmxspdr.py from %FILE% */
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif

#define GMXVERSION %GMXVERSION%
...
def main():
  ...
  # create an empty CCGMX object to get the version, assign prefix, etc
  decl = CCGMX(None, None, obj, pfx, hdrs)
  # supply two additional keys
  d = { "%FILE%":       str(fninp),
        "%GMXVERSION%": str(CCGMX.version) }
  # replace %PFX%, etc
  ...
  if CCGMX.version < 40010:
    cm = ccmdxv40
  elif CCGMX.version < 40510:
    cm = ccmdxv45
  else:
    print "this version %s of GROMACS is not supported" % CCGMX.version
    raise Exception
